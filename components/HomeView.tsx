  ...
  return (
    <div className="text-center space-y-16 pb-32 animate-slide-up">
      <header className="space-y-6 pt-10 relative">
        ...
        <PeachCoupleSVG
          action="blush"
          className="animate-float scale-150 mb-10"
        />
      ...
      <div className="glass p-10 rounded-[4rem] max-w-3xl mx-auto border-2 border-pink-100 shadow-2xl">
        <h4 className="font-cherry text-rose-500 text-2xl mb-8 uppercase">
          Your Journey Guide 💖
        </h4>
      ...
      {showMailbox && (
        <div className="animate-in slide-in-from-bottom duration-500 space-y-6 max-w-3xl mx-auto">
          <div className="flex justify-between items-center px-4">
            <h3 className="font-cherry text-rose-600 text-3xl">
              ayushi'S HEART REPLIES
            </h3>
            <button
          ...
          <div className="space-y-6">
            {replies
              .map((reply, i) => (
                <div
                  key={i}
                  className="glass p-10 rounded-[3rem] text-left border-l-[12px] border-rose-400 shadow-xl"
                >
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-sm font-black text-rose-500 uppercase tracking-[0.3em]">
                      {reply.dayTitle}
                    </span>
                  ...
                  <p className="text-2xl font-bold text-gray-700 italic font-quicksand">
                    "{reply.message}"
                  </p>
                </div>
              ))
      ...
      <div className="space-y-16">
        <div className="flex flex-col items-center space-y-4">
          <h2 className="text-5xl font-cherry text-rose-600 uppercase tracking-[0.3em] px-4">
            Secret Chapters
          </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 px-6">
          {VALENTINE_WEEK.map((day) => {
            ...
            return (
              ...
              >
                {!unlocked && (
                  <div className="absolute -top-5 -right-5 text-3xl bg-white p-4 rounded-3xl shadow-2xl z-20 border-2 border-pink-50 animate-bounce">
                    🔒
                  </div>
                )}
                <div
                  className={`text-8xl mb-8 transition-all duration-500 group-hover:scale-125 group-hover:rotate-12 ${!unlocked ? "grayscale blur-[2px] opacity-40" : "animate-wobble"}`}
                >
                  {day.icon}
                </div>
                <h3 className="font-black text-[#5D4037] text-2xl tracking-tighter uppercase mb-4">
                  {day.title}
                </h3>
                <div className="bg-white/50 px-6 py-2 rounded-full border border-white">
                  <p className="text-sm font-black text-rose-400 uppercase tracking-widest leading-none">
                    FEB {day.date}
                  </p>
                </div>
              </button>
