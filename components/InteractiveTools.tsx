...
const globalAudio = new Audio('https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3'); 
  ...
  return (
    <div className="glass p-5 rounded-[2.5rem] border-2 border-white shadow-xl space-y-3 w-full animate-slide-up">
      <div className="flex items-center space-x-3">
        <div className={`w-12 h-12 bg-pink-100 rounded-full flex items-center justify-center music-spin shadow-inner ${isPlaying ? '' : 'pause-animation'}`}>
          <span className="text-2xl drop-shadow-sm">🎵</span>
        </div>
        <div className="flex-1 overflow-hidden text-left">
          <p className="font-bold text-pink-600 truncate text-sm leading-tight">Ayushi's Lofi Paradise</p>
          <p className="text-[10px] text-pink-400 font-black uppercase tracking-widest">Romantic Chill Beats</p>
        </div>
      </div>
...
export const LoveCalculator: React.FC = () => {
  ...
  return (
    <div className="glass p-5 rounded-[2.5rem] text-center border-t-4 border-white shadow-xl flex flex-col justify-between h-full min-h-[160px] group">
      <h3 className="font-cherry text-pink-500 text-xs tracking-[0.2em] uppercase mb-2">LOVE SCANNER</h3>
      <div className="flex justify-center items-center space-x-2 mb-3">
        <span className="font-bold text-[10px] text-gray-500 bg-white/80 px-2 py-0.5 rounded-full shadow-sm">Ayushi</span>
        <span className="text-xl animate-pulse">❤️</span>
        <span className="font-bold text-[10px] text-gray-500 bg-white/80 px-2 py-0.5 rounded-full shadow-sm">Rishu</span>
      </div>
      <button 
        onClick={run} 
        disabled={calc}
      ...
      {res !== null && (
        <div className="animate-slide-up mt-2">
          <p className="text-2xl font-black text-rose-600 leading-none">{res}%</p>
          <p className="text-[9px] text-pink-400 font-black uppercase tracking-widest mt-1">SOULMATES! 🍑</p>
        </div>
      )}
    </div>
  );
...
export const MoodMeter: React.FC = () => {
  ...
  return (
    <div className="glass p-5 rounded-[2.5rem] text-center border-t-4 border-white shadow-xl flex flex-col justify-between h-full min-h-[160px]">
       <h3 className="font-cherry text-orange-500 text-xs tracking-[0.2em] uppercase mb-2">AYUSHI MOOD</h3>
...
export const FortuneCookie: React.FC = () => {
  ...
  return (
    <div className="glass p-5 rounded-[2.5rem] text-center border-t-4 border-white shadow-xl flex flex-col justify-between h-full min-h-[160px]">
      <h3 className="font-cherry text-amber-500 text-xs tracking-[0.2em] uppercase mb-2">PEACH COOKIE</h3>
      <div 
        onClick={crack} 
      ...
      {open ? (
        <p className="text-[10px] font-black text-amber-800 italic animate-slide-up leading-snug px-1">"{msg}"</p>
      ) : <p className="text-[9px] text-amber-400 font-black uppercase tracking-widest">TAP ME!</p>}
    </div>
  );
};
